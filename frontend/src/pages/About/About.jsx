import Map from '../../assets/Logo/map.jpg'
import '../../index.css'
import { MapPin } from 'lucide-react'
import {ToastContainer } from 'react-toastify'



const locations = [
  {
    title: 'Bengaluru office',
    timings: 'Mon-Sat 9am to 5pm.',
    address: '100, City Phase-10, 560101 IN',
  },
  {
    title: 'Head office',
    timings: 'Mon-Sat 9am to 5pm.',
    address: '12th Main Rd, 560101 IN',
  },
  {
    title: 'Karnataka office',
    timings: 'Mon-Sat 9am to 5pm.',
    address: '42, Residency Rd, 560102 IN',
  },
]


const About = () => {

    return(
        <div>
            <ToastContainer/>
            <div className='mx-auto max-w-7xl px-4'>
                {/* HERO MAP  */}
                <div className='flex flex-col space-y-8 pb-10 pt-12 md:pt-24'>
                    <div className='max-w-max rounded-full border bg-gray-50 p-1 px-3'>
                         <p className='text-xs font-semibold leading-normal md:text-sm'>About the company</p>
                    </div>
                    <p className='text-center text-3xl font-bold text-green-800 md:text-5xl md:leading-10'>
                        Predictive healthcare for a healthier you.
                    </p>
                    <p className='mx-auto max-w-4xl text-base text-gray-600 md:text-xl text-center'>
                        At HealthHub we offer our customers the ability to access the developments in medicare
                    </p>
                </div>
                <div className='w-full space-y-4'>
                    <img src={Map} alt="map img" 
                    className='h-[200px] w-full rounded-xl object-cover md:h-[400px]'
                    />
                </div>
                {/* locations */}

                <div className='my-8 flex flex-col gap-y-6 md:flex-row lg:justify-around '>
                    {locations.map((location) => (
                        <div key={location.title} className='flex flex-col space-y-3 md:w-2/4 lg:w-1/5'>
                         <MapPin className='h-5 w-5' />
                         <p className='w-full text-xl font-semibold text-gray-900'>
                            {location.title}
                         </p>
                         <p className='w-full text-base text-gray-700'>
                            {location.timings}
                         </p>
                         <p className='text-sm font-medium'>
                            {location.address}
                         </p>
                        </div>
                    ))}
                </div>
                <hr className='mt-20 text-gray-300' />
                
                {/* HIRING BANNER  */}

                <div className='flex flex-col items-center gap-x-4 gap-y-4 py-16 md:flex-row'>
                    <div className='space-y-6'>
                        <p className='text-sm font-semibold'>
                            Join our team &rarr;
                        </p>
                        <p className='text-3xl font-bold md:text-4xl'>
                            We&apos;re just getting started
                        </p>
                        <p className='text-gray-600 md:text-lg'>
                            Our philosophy is simple — hire a team of diverse, passionate people and foster a
                            culture that empowers you to do your best work.
                        </p>
                        <button 
                        type='button'
                        className='rounded-md bg-black px-3 py-2 text-sm font-semibold
                        text-white shadow-sm hover:bg-black/80 cursor-pointer'
                        >
                            Join Now
                        </button>
                    </div>
                    <div className='md:mt-0 mt-10 w-full'>
                        <img src="https://images.unsplash.com/photo-1605165566807-508fb529cf3e?ixlib=rb-4.0.3&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=2340&q=80" 
                        alt="Getting Started"
                        className='rounded-lg' 
                        />
                    </div>
                </div>
            </div>
        <hr className='mt-6 text-gray-300' />
        </div>
    )
}

export default About;